...
class TrajectoryLowHighModel(nn.Module):
    def __init__(self,embedding_dims=64, number_gcn_layers=1, dropout=0,obs_len=8, pred_len=12, n_tcn=5, num_heads=4):
        ...
        self.wt11_weighted_adjacency_matrices = SparseWeightedAdjacency(s_in_dims=2, T=4, embedding_dims=64)
        self.wt22_weighted_adjacency_matrices = SparseWeightedAdjacency(s_in_dims=2, T=2, embedding_dims=64)
        ...
        self.stsgcn_CD = SparseGraphConvolution(in_dims=2, embedding_dims=embedding_dims // num_heads, dropout=dropout)
        self.stsgcn2_CD = SparseGraphConvolution(in_dims=2, embedding_dims=embedding_dims // num_heads, dropout=dropout)
        ...
        self.tcns_CD = nn.ModuleList()
        self.tcns_CD.append(nn.Sequential(nn.Conv2d(4, pred_len, 3, padding=1),
            nn.PReLU()))
        for j in range(1, self.n_tcn):
            self.tcns_CD.append(nn.Sequential(nn.Conv2d(pred_len, pred_len, 3, padding=1),
                nn.PReLU()))

        self.tcns2_CD = nn.ModuleList()
        self.tcns2_CD.append(nn.Sequential(nn.Conv2d(2, pred_len, 3, padding=1),
            nn.PReLU()))
        for j in range(1, self.n_tcn):
            self.tcns2_CD.append(nn.Sequential(nn.Conv2d(pred_len, pred_len, 3, padding=1),
                nn.PReLU()))
    ...
    def forward(self, graph, obs_traj):
        ...
        G_S_CD, S_G_CD, S_E_CD = self.wt11_weighted_adjacency_matrices(graph1_CD.squeeze(), obs_traj1_CD.squeeze())
        G_S2_CD, S_G2_CD, S_E2_CD = self.wt22_weighted_adjacency_matrices(graph2_CD.squeeze(), obs_traj2_CD.squeeze())
        ...
        gcn_representation_CD = self.stsgcn_CA(graph1_CD, G_S_CD, S_G_CD, S_E_CD)
        gcn_representation2_CD = self.stsgcn2_CA(graph2_CD, G_S2_CD, S_G2_CD, S_E2_CD)
        ...
        features_CD = self.tcns_CD[0](gcn_representation_CD)
        for k in range(1, self.n_tcn):
            features_CD = F.dropout(self.tcns_CD[k](features_CD) + features_CD, p=self.dropout)

        features2_CD = self.tcns2_CD[0](gcn_representation2_CD)
        for k in range(1, self.n_tcn):
            features2_CD = F.dropout(self.tcns2_CD[k](features2_CD) + features2_CD, p=self.dropout)

        prediction = self.output(features + features_CA + features2_CA + features_CD + features2_CD)   # prediction=[N, Tpred, nums, 2]
